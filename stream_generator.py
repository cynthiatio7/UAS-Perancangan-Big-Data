...
def event_stream(total=1000, seed=42): # Added colon
    ...
    for n in range(1, total + 1): 
        ...
        if n in range(int(total * 0.20), int(total * 0.20) + 20):
            ...
            u = users[min(9999, len(users) - 1)]
            asset = ('payroll', 'database', 'confidential') 
            action = 'download'
            bytes_out = random.randint(1_000_000, 5_000_000) 
            src = '185.220.101.2' 
            status = 'success' 
            risk = 95
            ...
            u = users[min(22, len(users) - 1)] 
            asset = ('cust_db', 'database', 'restricted') 
            action = random.choice(['login', 'query', 'schema_discovery']) 
            bytes_out = random.randint(10_000, 100_000) 
            src = '45.77.21.13'
        ...
        yield {
            ...
            'event_time': (start + timedelta(seconds=n*10)).isoformat(), 
            'user_id': u['user_id'], 
            'dept': u['dept'], 
            'role': u['role'],
            ...
            'status': status,
            'bytes_out': bytes_out,
            'records_accessed': max(0, int(bytes_out / random.randint(1, 100))), 
            'latency_ms': max(1, int(random.gauss(120, 30))), 
